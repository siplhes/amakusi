...
<template>
  <div class="space-y-1.5">
    ...
    <div class="relative">
      <div v-if="icon" class="absolute left-3.5 top-1/2 -translate-y-1/2 pointer-events-none" :class="hasError ? 'text-red-600' : 'text-dark/30'">
      ...
      <textarea
        ...
        :class="[icon ? 'pl-11' : '', hasError ? 'border-red-400 focus:border-red-600 focus:ring-2 focus:ring-red-100' : 'border-dark/10 focus:border-primary focus:ring-2 focus:ring-primary/15']"
        @input="handleInput"
        @blur="handleBlur"
      />
      <input
    ...
    <Transition enter-active-class="transition-all duration-200" enter-from-class="opacity-0 -translate-y-1" enter-to-class="opacity-100 translate-y-0">
      <p v-if="hasError" :id="errorId" class="text-red-600 text-xs font-medium flex items-center gap-1" role="alert">
        <Icon icon="ph:warning-circle-fill" class="text-sm" aria-hidden="true" />
        {{ errors }}
      </p>
    </Transition>
  </div>
